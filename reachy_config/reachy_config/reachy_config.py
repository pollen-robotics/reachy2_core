...
class ReachyConfig:
    def __init__(self, custom_config_file_path="~/.reachy_config_custom"):
        self.custom_config_dir = os.path.expanduser(custom_config_file_path)
        # in package/default
        self.default_config_dir = os.path.dirname(os.path.realpath(__file__)) + "/../default"
        self.config = {}
        ...
        # list both dirs
        print("Custom config dir: ", os.listdir(self.custom_config_dir))
        print("Default config dir: ", os.listdir(self.default_config_dir))
        print("\n\n\n")
        # load every yaml from custom_config_dir
        for custom_config_file in os.listdir(self.custom_config_dir):
            # with open(custom_config_file) as f: dont forget to add prefix path
            # with open(self.custom_config_dir + "/" + custom_config_file) as f:
            ...
            # print(config)
            # self.config[basefilename] takes yaml dict value where basefilename is a safe get of file name without extension if any
            # self.config[os.path.splitext(custom_config_file)[0]] = yaml.load(f, Loader=yaml.SafeLoader)
